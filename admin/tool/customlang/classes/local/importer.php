...
class importer {
    ...
    public function import(array $files): void {
        ...
        $langfiles = $this->desploy_files($files);

        $this->process_files($langfiles);
    }

    /**
     * Desploy all files into a request folder.
     *
     * @param stored_file[] $files array of files to deploy
     * @return string[] of file paths
     */
    private function desploy_files(array $files): array {
        $result = [];
        // Desploy all files
     ...
     *
     * @param langstring[] $strings the strings to set
    ...
    private function import_strings(array $strings, stdClass $component): void {
        ...
        foreach ($strings as $newstring) {
            ...
            if ($this->can_save_string($customlang, $newstring, $component)) {
                // TODO: check if mlang_string::clean_text is necessary.
                $customlang->local = $newstring->text;
                $customlang->timemodified = $newstring->timemodified;
    ...
    private function can_save_string(?stdClass $customlang, langstring $newstring, stdClass $component): bool {
        ...
        switch ($this->importmode) {
            case self::IMPORTNEW:
                $result = empty($customlang->timecustomized);
                $message = 'notice_ignoreupdate';
                break;
            case self::IMPORTUPDATE:
                $result = !empty($customlang->timecustomized);
                $message = 'notice_ignorenew';
                break;
            case self::IMPORTALL:
                $result = true;
                break;
        }
        $errorlevel = ($result) ? notification::NOTIFY_SUCCESS : notification::NOTIFY_ERROR;
