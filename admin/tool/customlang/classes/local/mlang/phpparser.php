...
class phpparser {
    ...
    /**
     * @return singleton instance of phpparser
     */
        ...
        foreach ($tokens as $token) {
            ...
            if ($expect == 'STRING_TEXT') {
                if ($foundtype === T_CONSTANT_ENCAPSED_STRING) {
                    ...
                    throw new moodle_exception('Parsing error. Expected T_CONSTANT_ENCAPSED_STRING array item value at line '.$line);
                }
            }
    ...
    protected function decapsulate(string $text): string {
        ...
        if (substr($text, 0, 1) == "'" and substr($text, -1) == "'") {
            ...
            throw new moodle_exception('Parsing error. Unexpected quotation in T_CONSTANT_ENCAPSED_STRING in decapsulate(): '.$text);
        }
    }
}
